...
```
* `geom_jitter()` helps to give a random relocation for overlapping points, which improves the visualization of the data.
* `geom_qusirandom()` produces plots with random relocation for overlapping points just like jitter function, but it also gives a violin shape according to the density distribution. With the same hwy values, `geom_qusirandom()` arrange the points from the center line with random spacing. The random spacing further gives shape similar to the density distribution.
* `geom_beeswarm()` also creates plots with random relocation for overlapping points and violin shape. Comparing to `geom_qusirandom(), `geom_beeswarm()` arranges points with relatively equal spacing.
...
##make a color plot to show the distribution of cut within color
diamonds %>% 
  count(color, cut) %>%
  group_by(color) %>%
  mutate(rate = n / sum(n)) %>%
...
##make a color plot to show the distribution of color within cut
diamonds %>% 
  count(color, cut) %>%
  group_by(cut) %>%
  mutate(rate = n / sum(n)) %>%
