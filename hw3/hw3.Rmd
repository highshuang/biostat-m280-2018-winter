...
* Toyota makes received most citations (669548 citations).

0. How many different colors of cars were ticketed? Which color attracted most tickets?
```{r}
latix %>% distinct(Color) %>%
  count()
```

* There are 66 different colors of cars were ticketed.
```{r}
...
* Color black attracted most tickets.
...
latix %>% filter(Issue_Year == 2014) %>% 
  select(RP_State_Plate, Fine_amount) %>% 
  filter(!is.na(Fine_amount), !is.na(RP_State_Plate)) %>%
  ...
  select(Issue_Year, RP_State_Plate, Fine_amount) %>%
  ...
  filter(!is.na(Issue_Year), !is.na(RP_State_Plate)) %>%
  group_by(Issue_Year, RP_State_Plate) %>%
  summarize(sumFine = sum(Fine_amount, na.rm = TRUE))
```
