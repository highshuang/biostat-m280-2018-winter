...
latix %>% select(Ticket_number) %>% count() %>% collect()
  
  distinct(Ticket_number) %>%
  count()
  
  
  res <- dbSendQuery(db, "SELECT COUNT(DISTINCT Ticket_number) FROM latix")
...
```
* In total, there are 4044338 tickets in this data set. 
