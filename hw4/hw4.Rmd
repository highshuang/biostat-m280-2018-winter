...
* Select LAX flights data from 1998 to 2008. The above plot reflects the number of flights for seven weekdays in each year. The data from 1998 to 2008 reveal the similar pattern for relation between weekday and number of flights. Saturdays had least number of flights. From Monday to Friday, there were high demands for flights, but between these five weekdays, there are not significant difference. 

Saturday might sound like a popular — hence expensive — day to fly. But in truth, most travelers prefer to come back from vacation on Sunday to maximize their time away


In general, the number of flights achieved the maximun during summer time, and reached minimum during winter. Possible explanation is the long summer vacation encouraged students to travel during summer, and both Christmas time and low temperature in winter made peopel stay with their family rather than travel.

(d). Map top 10 destinations from LAX. Size of dots should reflect the number of flights from LAX to that destination.

```{r}
dest.from.LAX <- flights_tbl %>% 
      select(year, origin, dest) %>%
      filter(origin == "LAX" && year == 2007) %>%
      group_by(dest) %>%
      summarise(n = n()) %>% 
      arrange(desc(n)) %>%
      collect() %>%
    ...
    dest.location <- airports_tbl %>%
      select(faa, lat, lon) %>%
      filter(faa %in% dest.from.LAX$dest) %>%
      collect()
    get.dest.l <- left_join(dest.from.LAX, dest.location, 
                            by = c("dest" = "faa"))
    get.dest.l$lat <- as.numeric(get.dest.l$lat)
    get.dest.l$lon <- as.numeric(get.dest.l$lon)
    
    map3 <- get_map(location = 'united states', zoom = 4)
      ggmap(map3) + 
        geom_point(data = get.dest.l, aes(x = lon, y = lat, size = n), 
                   alpha = 0.5) +
        labs(x = "Longitude", y = "Latitude", 
             title = "Top 10 destinations from LAX in 2007") +
        geom_label_repel(data = get.dest.l, 
                         label = get.dest.l$dest, size = 2)
```
