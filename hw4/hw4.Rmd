...
air_ATL <- flights_tbl %>% filter(origin == "ATL", year <= 2008,
                                  year >= 1998) %>%
  left_join(airlines_tbl, by = c("uniquecarrier" = "code")) %>%
  group_by(uniquecarrier) %>%
  summarize(n = n()) %>% 
...
# plot the number of flights according to weekday in each year
ggplot(data = air_ATL, aes(x = as.factor(uniquecarrier), 
                              y = n, colour = as.factor(year),
                              group = as.factor(year))) +
  geom_line() +
  geom_point() +
  labs(
    x = "Carrier",
    y = "number of flights",
    title = "Top 10 carriers in ATL airport from 1998 to 2008"
  ) +
  scale_colour_discrete(name = "year") 
